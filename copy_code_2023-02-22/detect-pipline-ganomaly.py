...
def load_dataloader(source=0,
    ...
    #raise NotImplemented
    if GANOMALY:
        return dataset, bs, source, ganomaly
    else:
...
def Process_Prediction(pred=None,
    ...
    for i, det in enumerate(pred):  # per image
        ...
        if len(det):
            ...
            # Write results
            for *xyxy, conf, cls in reversed(det):
                #=================Alister add GAnomaly 2023-02-22============================================================================================
                #==================================
                ...
                if GANOMALY:
                    ...
                    global line_count
                    global noline_count
                    global normal_correct_detect_count
                    global abnormal_correct_detect_count
                    global abnormal_false_trigger_count
                    global abnormal_miss_noline_count
                    abnormal = 0
                    #imc2 = im0.copy()
                    c = int(cls)
                    
                    if float(conf) < float(ganomaly_conf) and filter_line_label==False:
...
if __name__ == "__main__":
    ...
    source = opt.source
    GANOMALY = opt.GANOMALY
    time_log = opt.time_log
    ganomaly_conf = opt.ganomaly_conf
    isize = opt.isize
    ganomaly_save_img = opt.ganomaly_save_img
    ganomaly_log = opt.ganomaly_log
    LOSS = opt.LOSS
    ...
    ganomaly = None
    if GANOMALY:
        dataset, bs, source, ganomaly = load_dataloader(source=source,
                                        nosave=False,
                                        imgsz=imgsz,
                                        stride=stride,
                                        pt=pt,
                                        GANOMALY=GANOMALY,
                                        isize=isize,
                                        ganomaly_save_img=ganomaly_save_img,
                                        ganomaly_log=ganomaly_log
                                        )
    else:
